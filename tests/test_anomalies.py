...
def is_ndvi_anomaly(ndvi_vals, threshold=-0.2):
    # anomaly if latest NDVI drops below threshold
    return np.asarray(ndvi_vals)[-1] < threshold
...
def test_ndvi_anomaly_flag():
    ndvi = [0.35, 0.31, 0.14, -0.25]  # last is below -0.2
    assert is_ndvi_anomaly(ndvi) is True
